...
describe('CodexCliAdapter', () => {
  it('falls through an inaccessible PATH alias to the current-user installation', async () => {
    const run = vi.fn(async (executable: string) => {
      ...
      return {
        exitCode:
          executable === 'C:/Users/example-user/AppData/Local/OpenAI/Codex/bin/current/codex.exe'
            ? 0
            : 1,
        stdout: '',
        stderr: '',
    ...
    await expect(
      discoverCodexCliExecutable({
        ...
        localCandidates: ['C:/Users/example-user/AppData/Local/OpenAI/Codex/bin/current/codex.exe'],
        run,
      }),
    ).resolves.toBe('C:/Users/example-user/AppData/Local/OpenAI/Codex/bin/current/codex.exe');
  });
