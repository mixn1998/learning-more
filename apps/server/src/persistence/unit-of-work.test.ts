...
describe('UnitOfWork [EQ-DATA-02]', () => {
  ...
  it('waits for a short write owned by another local unit of work', async () => {
    const root = await temporaryDataRoot();
    let releaseFirst!: () => void;
    let markFirstStarted!: () => void;
    const firstStarted = new Promise<void>((resolve) => {
    ...
    const first = createUnitOfWork({
      dataRoot: root,
      async faultInjector(point) {
        if (point !== 'journal:preparing') return;
        markFirstStarted();
        await firstMayFinish;
      },
    });
    const second = createUnitOfWork({ dataRoot: root });

    const firstCommit = first.execute({ transactionId: 'tx_external_first' }, (tx) =>
      tx.stageText('work/first.txt', 'first'),
    );
    await firstStarted;
    let secondFinished = false;
    const secondCommit = second
      .execute({ transactionId: 'tx_external_second' }, (tx) =>
        tx.stageText('work/second.txt', 'second'),
      )
      .then(() => {
        secondFinished = true;
      });
    ...
    await expect(
      readFile(path.join(root.absolutePath, 'work', 'second.txt'), 'utf8'),
