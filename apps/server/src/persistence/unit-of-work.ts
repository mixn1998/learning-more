...
export interface UnitOfWorkOptions {
  ...
  readonly writeLeaseWaitTimeoutMs?: number;
}
  ...
  return {
    execute<T>(
      ...
      const executeTransaction = async (): Promise<T> => {
        ...
        const lease = await acquireStoreWriteLease(options.dataRoot, undefined, {
          waitTimeoutMs: options.writeLeaseWaitTimeoutMs ?? 5_000,
        });
      ...
      const execution = transactionBarrier.then(executeTransaction);
      transactionBarrier = execution.then(
        () => undefined,
        () => undefined,
      );
      return execution;
