  ...
  const lines =
    phase === 'warmup'
      ? context.turnKind === 'opening'
        ? [
            '当前阶段是课前热身。环节目标是建立本课所需的理解背景：结合可用的知识地图事实，自然帮助学习者理解本课在当前模块和整门课程中的位置及学习意义；若本课是模块或课程起点，可相应扩大到模块或学科层级。当案例是本课的主要理解载体时，同时自然铺垫理解案例所需的关键链条与整体脉络，使后续事实、判断和机制有清晰位置。',
