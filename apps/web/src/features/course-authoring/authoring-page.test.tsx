...
describe('CourseAuthoring page', () => {
  ...
  it('keeps a terminal generation failure when the retained draft is only partial', async () => {
    const getOutlineSession = vi.fn().mockResolvedValue({
      outlineSessionId: 'session_01',
      resourceVersion: 4,
      state: 'assessment-ready',
      ...
      canGenerateCandidate: true,
      generationTaskId: 'task_01',
      candidateMarkdown: '# Partial candidate',
      messages: [],
    });
    const api = client({
      createOutlineSession: vi.fn().mockResolvedValue({
      ...
      getOutlineSession,
      streamGeneration: vi.fn().mockImplementation(async (_taskId, handlers) => {
        handlers.onEvent({ type: 'message.delta', data: { markdown: '# Partial candidate' } });
        handlers.onEvent({
          type: 'task.failed',
          data: { problem: { code: 'generation_interrupted' } },
        });
      }),
    });
    render(<AuthoringPage client={api} />);
    ...
    expect(screen.getByRole('button', { name: '重试生成' })).toBeEnabled();
    expect(screen.getByRole('heading', { name: 'Partial candidate' })).toBeInTheDocument();
    expect(getOutlineSession).toHaveBeenCalledTimes(1);
  });
