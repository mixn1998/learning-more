...
export function AuthoringPage(props: {
  ...
  const [loadedOutlineSessionId, setLoadedOutlineSessionId] = useState<string>();
  ...
  useEffect(() => {
    ...
    if (outlineSessionId === undefined) {
      setLoadedOutlineSessionId(undefined);
    ...
    void loadSession(outlineSessionId).then(
      () => {
        if (current) {
          setLoadedOutlineSessionId(outlineSessionId);
          setRestoreStatus('idle');
        }
      },
      () => {
  ...
  useEffect(() => {
    ...
    void api
          ...
          onEvent: (event) => {
            if (event.type === 'task.completed') {
              terminalRefresh ??= loadCandidateSession(outlineSessionId)
                .then((view) => {
                  if (view !== undefined) applySessionView(view);
                })
                .catch(() => undefined);
            } else if (event.type === 'task.failed' || event.type === 'task.cancelled') {
              const failureCode =
                event.type === 'task.failed'
                  ? candidateGenerationFailureFromEvent(event.data)
              ...
              terminalRefresh ??= loadCandidateSession(outlineSessionId, {
                ...
                .then((view) => {
                  dispatch({
                    type: 'generation-failed',
                    resourceVersion: view?.resourceVersion ?? resourceVersion,
                    failureCode,
                  });
                })
                .catch(() => {
                  dispatch({ type: 'generation-failed', resourceVersion, failureCode });
                });
              return;
            }
      ...
      .catch(async () => {
        ...
        if (recovered?.state === 'candidate-ready') {
  ...
  const sessionTransitionPending =
    props.initialOutlineSessionId !== undefined &&
    loadedOutlineSessionId !== props.initialOutlineSessionId;

  if (restoreStatus === 'loading' || (restoreStatus !== 'failed' && sessionTransitionPending)) {
    return (
