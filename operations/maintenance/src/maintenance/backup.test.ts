...
describe('backup triggers and retention', () => {
  ...
  it('never removes the final verified backup and preserves one upgrade point per schema', () => {
    ...
    const olderSameSchema = manifest({
      backupId: 'older-upgrade',
      createdAt: '2025-01-01T00:00:00.000Z',
      trigger: 'pre-upgrade',
    });
    const newerSameSchema = {
      ...preUpgrade,
      createdAt: '2025-01-01T01:00:00.000Z',
    };
    expect(
      selectBackupsForRetention(
        [olderSameSchema, newerSameSchema, latest],
        new Date('2026-07-13T00:00:00.000Z'),
      ).remove,
    ).toContain('older-upgrade');
  });

  it('keeps compact daily, weekly, and monthly recovery points', () => {
    ...
    expect(decision.keep).toEqual(
      expect.arrayContaining(['backup-0', 'backup-1', 'backup-2', 'backup-13']),
    );
  ...
  it('journals retention before deletion and resumes safely after a crash', async () => {
    ...
    await expect(readFile(path.join(journalDirectory, journalName!), 'utf8')).rejects.toMatchObject(
      {
        code: 'ENOENT',
      },
    );
