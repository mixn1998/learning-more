...
export function selectBackupsForRetention(
  ...
  const protectedUpgradeSchemas = new Set<number>();
  for (const manifest of eligible) {
    if (
      (manifest.trigger === 'pre-upgrade' || manifest.trigger === 'pre-migration') &&
      !protectedUpgradeSchemas.has(manifest.schemaVersion)
    ) {
      protectedUpgradeSchemas.add(manifest.schemaVersion);
      keep.add(manifest.backupId);
    }
  }
  keepNewestBuckets(eligible, keep, dayKey, 3);
  keepNewestBuckets(eligible, keep, weekKey, 2);
  keepNewestBuckets(eligible, keep, monthKey, 2);
  return {
    keep: sorted
...
export async function readBackupManifests(backupRoot: string): Promise<BackupManifest[]> {
...
async function collectUnreferencedObjects(backupRoot: string): Promise<void> {
  const manifests = await readBackupManifests(backupRoot);
