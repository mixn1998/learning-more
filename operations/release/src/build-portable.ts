...
async function copyProductionDependencies(
  ...
  for (const dependencyName of Object.keys(manifest.dependencies ?? {}).sort()) {
    if (externalPackages !== undefined && !externalPackages.has(dependencyName)) continue;
...
export function buildPortableStartCommand(): string {
  const waitForReady = [
    ...
    "    $web = Invoke-RestMethod -Uri ('http://127.0.0.1:43119/build-meta.json?startup=' + [DateTime]::UtcNow.Ticks) -TimeoutSec 1",
    "    if ($ready.status -eq 'ready' -and $web.buildId -eq $ready.buildId) {",
...
export function resolvePortableBuildPaths(
  ...
  return {
    outputRoot,
    workRoot,
    expandedRoot: path.win32.join(outputRoot, 'portable', 'Learning MORE'),
  };
}

export function portableWebBuildSettings(
  buildId: string,
  workRoot: string,
): Readonly<{
  buildId: string;
  ...
  const outputRoot = path.win32.join(workRoot, 'web-dist');
  return {
    buildId,
    outputRoot,
    environment: {
...
export async function buildPortableRelease(
  ...
  const webBuild = portableWebBuildSettings(buildId, workRoot);
  runPnpm(projectRoot, ['--filter', '@learning-more/web', 'build'], webBuild.environment);
  const webBuildMeta = JSON.parse(
    await readFile(path.join(webBuild.outputRoot, 'build-meta.json'), 'utf8'),
  ) as { buildId?: unknown };
  if (webBuildMeta.buildId !== buildId) {
    throw new Error(
      `portable_web_build_identity_mismatch:${String(webBuildMeta.buildId)}:${buildId}`,
    );
  }
  ...
  const externalServerPackages = new Set(['fastify', 'pdfjs-dist']);
  execFileSync(
    process.execPath,
    [
      ...
      '--target=node24',
      '--packages=bundle',
      ...[...externalServerPackages].map((packageName) => `--external:${packageName}`),
      ...[...externalServerPackages].map((packageName) => `--external:${packageName}/*`),
      '--minify',
      '--legal-comments=none',
  ...
  await copyProductionDependencies(projectRoot, serverRoot, externalServerPackages);
  ...
  await copyRuntimeTree(webBuild.outputRoot, path.join(expandedRoot, 'app', 'web'));
  await copyRuntimeTree(
    path.join(projectRoot, 'operations', 'maintenance', 'dist'),
    path.join(expandedRoot, 'operations', 'maintenance', 'dist'),
  );
  await cp(
    path.join(projectRoot, 'operations', 'maintenance', 'package.json'),
    path.join(expandedRoot, 'operations', 'maintenance', 'package.json'),
  );
  await mkdir(path.join(expandedRoot, 'operations'), { recursive: true });
  await writeFile(
    path.join(expandedRoot, 'operations', 'learning-more.cmd'),
    '@echo off\r\n"%~dp0..\\runtime\\node.exe" "%~dp0maintenance\\dist\\main.js" %*\r\n',
    'utf8',
  );
