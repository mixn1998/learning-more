...
> **完成更新（2026-07-14）：** 11 个任务与全需求清单已全部实现并勾选。正式产品由 Launcher 在 `http://127.0.0.1:43119/` 单一来源托管；内部后端故障不关闭网站，可由运行中心诊断、重连和自动恢复。最终证据见 `docs/superpowers/reports/2026-07-14-frontend-final-acceptance.md`。
...
- 任一任务使既有 560 个测试、4 条核心 E2E、5 条 Runtime E2E 或 75 条等价断言回退，不得标记完成。
...
- [x] 架构检查 237 个 data key 通过，无禁止依赖；
- [x] 75 条等价断言全部实现并通过；
- [x] 163 个测试文件、560 个测试全部通过；
...
- [x] 22 组关键交互回归全部通过；
- [x] 29×3 排版/间距审计全部通过；
- [x] 29×3 视觉完整性审计与检测器自测全部通过；
- [x] 统一、自包含的样稿审计命令；
- [x] 87 张 HTML 权威截图基线；
- [x] 24 张九模式交互派生基线及对应 React 对照；
- [x] React 正式视觉回归基线。
...
- [x] 以实际文件系统固定 29 个 HTML 清单，并建立测试防止数量和路径漂移；
- [x] 修复课程规划 pill 的 9 个行高失败，不改变样稿信息架构；
- [x] 修复移动端页面宽度/裁切问题；
- [x] 修复周报日期标签的 14 个行高失败；
- [x] 把交互测试中的导航等待改为目标 URL/稳定标记断言；
- [x] 把计划流每日目标动作改为数值 input 的真实填写方式；
- [x] `run-all-audits.mjs` 自行启动临时静态服务、探测端口、顺序执行全部审计并可靠退出；
- [x] 在根 `package.json` 增加 `ui-samples:verify` 与 `visual:test`；
- [x] 按设计规格中的 29 行映射建立稳定 state ID、Route/Fixture、准备步骤和关键组件区域；
- [x] 生成桌面 1440×1000、平板 1024×768、移动端 390×844 共 87 张 HTML 基线；
- [x] 固定 locale、时区、DPI、颜色方案、Mock 数据、时间、ID、动画和字体加载；
- [x] 截图前验证 Times New Roman、SimSun 和 SimHei 可用；
- [x] 基线写入 `tests/visual/baselines`，运行产物写入 `artifacts/visual`；
- [x] README 记录权威命令、29 页/87 张核心基线、24 张模式派生基线和失败定位方式。
...
- Create: `apps/web/src/visual/design-system-fixtures.tsx`
- Create: `apps/web/src/visual/design-system-fixtures.css`
- Create: `tests/visual/design-system-components.spec.ts`

**Checklist**

- [x] 将样稿颜色、间距、圆角、阴影、层级、断点迁入唯一 token；
- [x] 将九模式 accent/accent-dark/tint/motif 与样稿完全统一；
- [x] 模式色与 success/warning/danger/readonly/degraded 状态色解耦；
- [x] 实现 Page、Stack、Inline、Grid、SidebarLayout、Panel、Card、Toolbar、SectionHeader；
- [x] 实现 Button、Badge/Pill、Tabs、Dialog、Toast、Field、输入控件和统一内容状态；
- [x] 保留并视觉升级 StatusBanner，不改变现有运行状态语义；
- [x] Dialog 实现焦点陷阱、Escape、背景隔离和关闭后焦点恢复；
- [x] Tabs 实现键盘、ARIA 关联和选中态；
- [x] 实现 `AiContent` 与受净化 Markdown；
- [x] AI 标题计算字体为 SimHei/黑体；
- [x] AI 英文正文计算字体为 Times New Roman；
- [x] AI 中文正文计算字体为 SimSun/宋体；
- [x] 代码保持统一等宽字体；
- [x] AI 正文、标题、列表、引用、表格和代码块使用统一行高与间距 token；
- [x] App Shell 迁移顶栏、品牌、全局导航、Runtime 状态和写入阻断视觉；
- [x] 1440、1024、390 三视口无溢出和遮挡；
- [x] 不在 `packages/ui` 引入路由、API 或领域类型。

**Tests**

- [x] `packages/ui` 覆盖 variants、disabled/loading、ARIA、键盘和焦点；
- [x] `AiContent` 覆盖中文、英文、中英混排、标题、列表、引用、表格、行内代码和代码块；
- [x] 建立设计系统两个 Visual Fixture，与样稿 1–2 对照；
- [x] 运行状态、语义状态组和模式身份卡三类关键组件在三视口通过 0.1% 裁切门禁；
- [x] App Shell 建立主页/移动端/运行降级视觉状态。
...
- [x] 所有 Client 通过 `apiRequest` 统一解析 success/Problem/ETag；
- [x] 所有 Response 使用共享 Schema，移除手写 `as ReturnType` 和未校验 JSON；
- [x] 404、503、stale、rebuilding 使用显式 union，移除 `undefined as T`；
- [x] CSRF、page instance、If-Match、correlation 和 content type 由 Client 统一注入；
- [x] 每个可重试写操作从 `use-command-attempt` 获取稳定 idempotency key；
- [x] 失败重试复用身份，成功或用户明确放弃后才旋转；
- [x] 增加大纲修订 Client；
- [x] 增加课程摘要 Client；
- [x] 增加关闭事务读取和重试 Client；
- [x] 增加 Runtime 诊断 Client；
- [x] SSE 继续支持 Last-Event-ID、snapshot reset、取消和 transient reconnect；
- [x] Client 单元测试覆盖重复命令、ETag 冲突、404/503 union、SSE 恢复和取消。
...
- [x] 新增权威首页聚合读取，返回未确认大纲、正式课程、活动会话、推荐课节和周计划；
- [x] `HomeRoute` 真实加载数据，不再依赖默认空 props；
- [x] 保留当前 `selectContinueTarget` 优先级，不重新实现；
- [x] 无课程时只显示创建入口，不显示伪继续操作；
- [x] 草稿和正式课程分区且链接到真实路由；
- [x] 九模式同规格入口、standard 第一且默认选中；
- [x] 点击任一玩法后，全局 accent/accent-dark/tint、正式 `--lm-*` token、徽标、输入、主按钮和选中态与样稿同步；
- [x] 周历、课程切换、成功/错误提示与样稿一致；
- [x] 加载、空态、错误、degraded 和 version mismatch 完整。

**Checklist — 建档**

- [x] 保留当前 Authoring reducer 和 SSE 状态机；
- [x] 主页选择模式不创建会话，提交后只创建一次；
- [x] OutlineSession 工作台不重复显示模式选择和主题输入；
- [x] 阅读研讨真正上传并摄取 PDF/TXT/MD，展示解析/失败/来源状态；
- [x] 起点评估、候选、失败草稿、重生成、冲突、确认和刷新恢复全部迁入正式布局；
- [x] 所有 AI 消息和候选大纲使用 `AiContent`；
- [x] 九模式共享同一组件和状态机，只改变统一主题变量；
- [x] 默认主页 3 个视口和八种非默认模式 24 个交互截图均通过全页 0.3% 门槛；
- [x] 样稿 4–12 在三个视口逐一通过。

**Checklist — 正式课程**

- [x] 展示正式大纲、课节顺序、版本、状态、课程模式和学习进度；
- [x] 大纲修订接真实 `outline-revisions`，冲突后可加载权威版本；
- [x] 版本记录和只读历史视图完成；
- [x] 关闭态锁定编辑，保留查看大纲、课节记录和课程 Review；
- [x] 课程 Review 使用最终大纲和课节 Review 的服务端权威结果；
- [x] 课程生命周期确认和结果状态使用共享 Dialog/Toast；
- [x] 正式大纲、修订说明和课程 Review 使用 `AiContent`；
- [x] 样稿 13–16、25 在三个视口逐一通过。

**Tests**

- [x] 保留并通过现有 Home/Authoring/Course 组件测试；
- [x] 增加首页聚合空态、继续学习、草稿/课程分区和错误恢复测试；
- [x] 增加材料摄取成功/失败、修订冲突和关闭态只读测试；
- [x] E2E 覆盖首页 → 建档 → 确认 → 修订 → 课节导航 → 课程状态闭环；
- [x] 全页视觉差异 ≤0.3%，关键组件 ≤0.1%。
...
- [x] 新增 schedule move/resize/lock/remove 请求和响应 Schema；
- [x] 新增计划流 pause/resume/reflow/end 请求和响应 Schema；
- [x] HTTP 路由调用现有领域命令，不复制规划规则；
- [x] 所有写操作支持 idempotency、page instance、If-Match 和 Problem；
- [x] 增加成功、冲突、重复请求和恢复集成测试；
- [x] OpenAPI 与 Schema 同步。

**Checklist — 正式页面**

- [x] 七日栏、日期和待规划筛选互斥；
- [x] 计划卡展示课程、课节、日期、时段、来源、锁定和派生状态；
- [x] 手工排期、改期、调整时长、取消和锁定接真实命令；
- [x] 完成/放弃课节不可重新排入活动计划；
- [x] 放弃课节取消有效排期，恢复不自动恢复旧排期；
- [x] 四步计划流向导保留约束、预览零写入、确认原子写入；
- [x] 支持暂停、恢复、重排和结束管理；
- [x] 手工锁定项不被重排；
- [x] 冲突时保留草稿和约束，展示权威新日程；
- [x] AI 计划说明使用 `AiContent`；
- [x] 样稿 17–18 在三个视口逐一通过。
...
- [x] 未开始页展示权威目标、核心知识点和预计时间，点击开始前零写入；
- [x] 已放弃页展示已学/剩余、查看记录和恢复，不混入全局历史；
- [x] 保留当前会话 reducer、租约、暂停/恢复和窗口生命周期；
- [x] 服务端 Session View 返回完整消息序列、生成状态和关闭准备数据；
- [x] 对话渲染完整用户/AI 消息，不只累积一次 assistant 文本；
- [x] AI 消息、补充会话和流式 Markdown 统一使用 `AiContent`；
- [x] SSE transient reconnect、恢复快照、终态和错误均有可见状态；
- [x] 停止生成保留草稿引用和输入；
- [x] 第二窗口只读、写入权转移后原窗口立即停止计时。

**Checklist — 结束与 Review**

- [x] 移除固定 hash、固定 message ID 和固定 end intent；
- [x] 关闭输入由服务端权威会话快照或 prepare endpoint 提供；
- [x] 关闭事务展示 pending/completed/failed，并接真实 retry；
- [x] 同一快照重试幂等，失败不产生完成事实；
- [x] Review 后台完成不依赖页面存活；
- [x] 在页完成时打开正文可滚动、头尾固定的 Review Dialog；
- [x] 离页后完成时下次只提示一次并打开 Review Tab；
- [x] Dialog 只提供查看课节记录和返回课程大纲；
- [x] 阶段/最终 Review 使用 `AiContent` 且最终 Review 保持不可变。

**Checklist — 记录**

- [x] 顶层“学习对话/课时 Review”双 Tab；
- [x] 对话 Tab 内切换原始与补充会话；
- [x] 历史记录只读，不出现修改和错误的补充学习入口；
- [x] Calendar/History 深链打开正确课程、课节和 Review Tab；
- [x] 长 Markdown、表格和代码在三个视口不裁切；
- [x] 样稿 19–23 在三个视口逐一通过。

**Tests**

- [x] 保留现有 Session、窗口生命周期、记录和 E2E 测试；
- [x] 增加完整消息 hydration、SSE 恢复、关闭事务 retry、离页通知测试；
- [x] 增加键盘/焦点/只读/双窗口测试；
- [x] 全页视觉差异 ≤0.3%，Review Dialog ≤0.1%。
...
- [x] Route 对历史、统计、日历、周数据和周报分别处理 loading/error/stale/rebuilding；
- [x] 单个读模型失败不令整个页面永久 loading；
- [x] 三个顶层入口保持同级，不新增课程聚合式历史入口；
- [x] 统计 ViewModel 展示总览、分布、互动、趋势、样本量、排除项和来源时间；
- [x] 互动统计只使用结构化关键互动口径；
- [x] 历史分页追加，不替换已加载事实；
- [x] 筛选日期后不残留旧结果；
- [x] 课程摘要抽屉接 `/courses/:courseId/summary`；
- [x] 月历按本地日期聚合并支持上一月/下一月；
- [x] 完成条目深链到课程和权威 Review；
- [x] 周报默认折叠；
- [x] 展开周报展示冻结状态、结构化周统计和 AI Markdown，不再输出 JSON/`pre`；
- [x] 周报使用 `AiContent`；
- [x] 样稿 24、26、27 在三个视口逐一通过。

**Tests**

- [x] 404/503/stale/rebuilding、部分失败和重试测试；
- [x] 月份切换、本地日期、空日期、分页和深链测试；
- [x] 周报默认折叠、展开、无报告和冻结态测试；
- [x] 数据定义变更同步相关文档和等价断言。
...
- [x] Route 并行读取档案、证据和当前画像并分别处理错误；
- [x] 刷新命令使用稳定 idempotency key；
- [x] preparing/generating 按 versionId 轮询，完成/失败/卸载后停止；
- [x] 刷新失败通过 `finally` 解除 busy，保留上一版画像；
- [x] 样本不足不生成模板化人格洞察；
- [x] 失败态显示可重试原因，不显示伪画像；
- [x] 复合证据抽屉展示来源组、时间、限制和反向证据，不泄漏内部置信标签；
- [x] 洞察维度和数量保持自由，不新增固定分类；
- [x] 标题、摘要、洞察和证据说明使用 `AiContent`；
- [x] 样稿 28 在三个视口通过。

**Tests**

- [x] 保留现有画像状态和证据测试；
- [x] 增加轮询完成、轮询失败、卸载取消、旧版保留和重试测试；
- [x] E2E 覆盖首次画像、增量刷新、失败保留和证据展开；
- [x] 全页视觉差异 ≤0.3%，证据抽屉 ≤0.1%。
...
- [x] 正式展示实例、构建、协议、数据根、Provider、模型、健康和最后检查时间；
- [x] 保留当前四阶段重连和 capability 安全边界；
- [x] Provider 验证失败不改变当前值；
- [x] API Key 不进入普通持久化、日志或 DOM 回显；
- [x] 接入 `/api/v1/runtime/diagnostics`；
- [x] 展示净化后的诊断结果、错误码和修复建议；
- [x] 错端口/错数据根/错 instanceId 不显示为健康；
- [x] 外部端口占用不提供强杀动作；
- [x] 协议/构建不匹配与全局写入阻断保持同步；
- [x] 诊断代码使用等宽字体，不套用 AI 字体；
- [x] 样稿 29 在三个视口通过。
...
- [x] 29 个样稿状态都有 React Route/Fixture、三视口和稳定断言；
- [x] 87 张 HTML 基线全部可读取；
- [x] 24 张九模式交互派生基线及 React 对照全部通过；
- [x] 87 个对应 React 迁移状态全部建立长期基线；
- [x] 业务动态的 loading、empty、error、conflict、rebuilding、readonly 和生成状态建立关键组件基线；
- [x] 截图环境固定字体、时区、locale、时间、ID、DPI、动画和网络；
- [x] 全页 `threshold: 0.15`、`maxDiffPixelRatio: 0.003`；
- [x] 关键组件 `maxDiffPixelRatio: 0.001`；
- [x] 失败输出 expected/actual/diff、比例、状态名和 trace；
- [x] 禁止自动批量更新快照。

**Checklist — 视觉与字体**

- [x] 三视口零横向溢出；
- [x] 零正文裁切、控件遮挡、同组高度漂移和死按钮；
- [x] 九模式身份贯穿建档、课程、学习、Review 和历史；
- [x] 所有 AI 输出入口静态扫描为 `AiContent`；
- [x] AI 标题、中文、英文和代码计算字体全部正确；
- [x] 标题、正文、列表、引用、表格和代码块行距/间距统一；
- [x] Feature 不覆盖共享字体、字号、行高和视觉 token。

**Checklist — 无障碍**

- [x] 全局导航、Tabs、Dialog、Toast、表单和动态状态键盘可操作；
- [x] 可见焦点完整；
- [x] Dialog 焦点陷阱和关闭恢复完整；
- [x] `aria-live`、`aria-busy` 和只读状态正确；
- [x] 颜色不是唯一状态表达；
- [x] 200% 缩放可用；
- [x] reduced motion 生效；
- [x] 浏览器控制台零未处理错误和 Promise rejection。
...
- [x] 逐项复核本计划所有复选项和证据；
- [x] 确认 29 个状态、87 张 HTML 基线、24 张模式交互派生基线和全部 React 基线一致；
- [x] 确认业务 E2E、Runtime E2E、75 条等价断言和全仓门禁为绿色；
- [x] 确认不存在生产 `demoMode`、样稿 iframe、内联业务 Fixture 或第二套 token；
- [x] 确认所有 Client 使用共享合同、稳定命令身份和统一错误模型；
- [x] 确认所有 AI 生成内容使用统一字体和排版；
- [x] 确认三视口和无障碍零阻断问题；
- [x] 文档记录最终版本、命令、基线更新规则和维护责任；
- [x] 生成最终验收报告，包含通过项、证据路径和已知非阻断限制；
- [x] 正式将 `apps/web` 标记为唯一产品 UI，HTML 保留为视觉参考。
...
- [x] 29 个样稿状态的正式 React 页面；
- [x] 87 张 HTML 权威基线、24 张模式交互派生基线和对应 React 长期基线；
- [x] 共享 token、组件、布局、九模式视觉身份和 App Shell；
- [x] 全 AI 内容标题黑体、中文宋体、英文 Times New Roman、代码等宽；
- [x] 首页真实聚合数据；
- [x] 材料上传/摄取、正式大纲修订和版本视图；
- [x] 规划改期/时长/取消/锁定及计划流管理跨层接口；
- [x] 完整会话消息、权威关闭输入、事务恢复和 Review 通知 UX；
- [x] 课节记录完整只读导航；
- [x] 历史统计信息架构、月历、课程摘要和折叠周报；
- [x] 画像轮询、失败回收和旧版保留；
- [x] Runtime 诊断页面；
- [x] 三视口、键盘、焦点、200% 缩放和动态状态无障碍；
- [x] 全页 ≤0.3%、关键组件 ≤0.1%、零溢出/裁切/死控件；
- [x] 发布级全量验收和文档同步。
