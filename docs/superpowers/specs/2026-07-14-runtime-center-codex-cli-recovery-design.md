...
本机最终验收时 Codex CLI 为 `0.144.0-alpha.4`；实时目录返回 Sol、Terra、Luna、GPT-5.5、GPT-5.4、GPT-5.4-Mini、GPT-5.3-Codex-Spark 共 7 个可见模型。其中 Sol 支持 `low`、`medium`、`high`、`xhigh`、`max`、`ultra` 六档推理强度，默认 `low`。这些值会随 CLI 与账号能力变化，只作为验收证据，不写入产品静态目录。
...
- Provider 卡片严格区分“当前使用”和“健康但未连接”：只有当前 Provider 可以显示“已连接/当前使用”，其他健康接口显示“可用 · 未连接”。
- Codex CLI 模型选择器只显示返回目录中的模型名；默认推理强度在独立的推理强度字段旁标注为“模型默认，可调整”，避免被误解为只能使用默认档位。
- 首次读取、切换、重连、检查和登录过程统一显示“连接中”；页面打开期间每 5 秒同步当前 Provider 状态和模型目录，并展示最近同步时间。
- 选择模型后，推理强度选择器只显示该模型支持的值。
- 没有模型时禁用切换操作，并明确显示探测失败原因。
- 未登录时显示“登录 Codex”；启动后显示“等待浏览器验证”，验证成功后自动加载真实模型，不要求手动刷新。
- “启动并检查”只用于需要显式启动探测的非 Codex Provider；Codex CLI 配置区不显示该按钮，避免被误解为“启动模型”。“重新连接”重放已保存配置并重新探测当前接口。
...
- 页面只显示本次实际 CLI 目录返回的型号；任何型号在目录中消失后，刷新页面也同步消失，不保留历史硬编码选项。
