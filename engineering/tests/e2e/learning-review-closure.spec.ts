...
async function restartServer() {
  ...
  try {
    ...
    // The recovery fixture may already have stopped the server before mutating the store.
  }
...
async function createCourse(page: Page): Promise<{ courseId: string; lessonIds: string[] }> {
  ...
  await page.getByLabel('补充需求').fill('Use two focused lessons');
  await expect(completeAssessment).toBeEnabled();
  await completeAssessment.click();
...
test('[EQ-LESSON-03] completes learning lifecycle, immutable lesson Reviews, and course closure', async ({
  ...
  await completeLesson(page, courseId, lessonIds[0]!);
  await page.goto(`/courses/${courseId}/lessons/${lessonIds[0]!}/record?tab=review`);
  await expect(page.getByLabel('权威课时 Review')).toContainText('本课方法论启示');
  await page.getByRole('tab', { name: '学习对话' }).click();
  ...
  await expect(supplementaryInput).toBeEnabled();
    ...
    supplementary: [
      {
        ...
        messages: expect.arrayContaining([
          expect.objectContaining({ role: 'user', markdown: 'Explore a related example' }),
        ]),
      },
    ],
  });
  ...
  await expect(page.getByLabel('补充学习对话')).toContainText('Explore a related example');
  ...
  expect(firstFinalReview.id).not.toBe('');
  expect(firstFinalReview.artifactRef).not.toBe('');
  expect(
    completed.filter(
      (document) =>
