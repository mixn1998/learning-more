...
describe('test suite policy', () => {
  it('keeps performance and recovery drills out of daily test commands', () => {
    ...
    expect(packageJson.scripts['test:ci']).toContain('--exclude engineering/tests/performance/**');
    expect(packageJson.scripts['test:ci']).toContain('--exclude engineering/tests/recovery/**');
  });

  it('composes release verification without recursively invoking daily verification', () => {
    expect(packageJson.scripts.verify).toBe('node engineering/verification/verify-change.mjs');
